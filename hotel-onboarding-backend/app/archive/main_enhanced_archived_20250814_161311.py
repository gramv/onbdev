...
# Prefer .env.test when present; fall back to default .env
from pathlib import Path as _Path
_env_test = _Path(__file__).resolve().parents[1] / ".env.test"
if _env_test.exists():
    load_dotenv(dotenv_path=_env_test)
else:
    load_dotenv()
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    for error in exc.errors():
        ...
        errors_list.append({"loc": error.get("loc", []), "msg": error_msg, "type": error.get("type")})
    
    # Return standardized error with field_errors so frontend can display specifics
    content = ResponseFormatter.error(
        message="Request validation failed",
        error_code=ErrorCode.VALIDATION_ERROR,
        status_code=422,
        detail=errors_list or "One or more request fields are invalid",
...
# Debug endpoint to expose exact pydantic errors for application payloads
@app.post("/debug/validate-application")
async def debug_validate_application(payload: dict):
    try:
        _ = JobApplicationData(**payload)
...
async def login(request: Request):
    ...
    try:
        ...
        # Generate token - MVP: manager-only login
...
async def refresh_token(current_user: User = Depends(get_current_user)):
    ...
    try:
        # Generate new token based on user role - MVP: manager-only refresh
...
async def approve_application(
    ...
    try:
        ...
        # Create onboarding session (be resilient to schema issues)
        try:
            ...
            logger.warning(f"Onboarding session creation failed, continuing without session: {create_session_error}")
            onboarding_session_data = {
                "token": None,
        ...
        base_url = os.getenv("FRONTEND_URL", "http://localhost:3000")
        onboarding_url = f"{base_url}/onboard?token={onboarding_session.token}" if onboarding_session.token else None
...
@app.post("/apply/{id}")
async def submit_job_application(id: str, application_data: Dict[str, Any]):
    """Submit job application to Supabase with explicit validation and detailed errors"""
    try:
        # Normalize payload for MVP compatibility
        payload: Dict[str, Any] = dict(application_data or {})
        # Normalize shift preference
        allowed_shifts = {"morning", "afternoon", "evening", "night", "flexible"}
        sp = str(payload.get("shift_preference", "")).strip().lower()
        if sp not in allowed_shifts:
            payload["shift_preference"] = "flexible"
        # Normalize employment history dates
        eh_list = payload.get("employment_history") or []
        if isinstance(eh_list, list):
            for eh in eh_list:
                if isinstance(eh, dict):
                    if not eh.get("from_date"):
                        eh["from_date"] = "Present"
                    if not eh.get("to_date"):
                        eh["to_date"] = "Present"
        payload["employment_history"] = eh_list

        # Explicit validation to surface field-level errors
        try:
            validated: JobApplicationData = JobApplicationData(**payload)
        except Exception as e:
            # Build field errors from pydantic
            ...
            try:
                ...
                pass
            return JSONResponse(status_code=422, content=ResponseFormatter.error(
                message="Request validation failed",
                error_code=ErrorCode.VALIDATION_ERROR,
                status_code=422,
        ...
        for app in existing_applications:
            app_position = _safe_get(app, "position")
            app_status = _safe_get(app, "status")
            app_id = _safe_get(app, "id")
            if hasattr(app_status, "value"):
                app_status = app_status.value
            # Idempotent success when duplicate pending exists
            if app_position == validated.position and str(app_status).lower() == "pending":
                return {
                    "success": True,
                    "duplicate": True,
                    "message": "You have already submitted an application for this property and position.",
                    "application_id": app_id,
                    "property_name": property_obj.name if property_obj else None,
                    "position_applied": f"{validated.position} - {validated.department}",
        ...
        if validated.employment_history:
            for emp in validated.employment_history:
        ...
        job_application = JobApplication(
            ...
            applicant_data={
                ...
                "work_authorized": validated.work_authorized,
                "sponsorship_required": validated.sponsorship_required,
                "start_date": validated.start_date,
                "shift_preference": validated.shift_preference,
                "employment_type": validated.employment_type,
                "experience_years": validated.experience_years,
                "hotel_experience": validated.hotel_experience,
...
async def get_onboarding_session(token: str):
    ...
    try:
        ...
        if token_data['employee_id'].startswith('test-emp-'):
            ...
            try:
                # Get employee data using Supabase client directly
                employee_response = supabase_service.client.table('employees').select('*').eq('id', token_data['employee_id']).execute()
                
                if employee_response.data and len(employee_response.data) > 0:
                    employee = employee_response.data[0]
                else:
                    # Employee not found in database - return error
                    return not_found_response("Employee not found in database")
                
                # Get property data  
                property_response = supabase_service.client.table('properties').select('*').eq('id', employee['property_id']).execute()
                property_data = property_response.data[0] if property_response.data else {
                    'id': employee['property_id'],
                    'name': 'Unknown Property',
                    'address': 'Address not available'
                }
                
                # Get progress data
                progress_response = supabase_service.client.table('onboarding_progress').select('*').eq('employee_id', token_data['employee_id']).execute()
                completed_steps = []
                if progress_response.data:
                    completed_steps = [p['step_id'] for p in progress_response.data if p.get('completed')]
                    
            except Exception as e:
                logger.error(f"Database error in get_onboarding_session: {e}")
                # For database errors, return a proper error response
                return error_response(
                    message="Failed to load onboarding session from database",
                    error_code=ErrorCode.DATABASE_ERROR,
                    status_code=500
                )
