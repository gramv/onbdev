...
async def add_signature_to_i9(
    ...
    """Add digital signature to I-9 PDF"""
    try:
        pdf_base64 = signature_data.get('pdf_data')
        signature_base64 = signature_data.get('signature')
        signature_type = signature_data.get('signature_type', 'employee_i9')
        page_num = signature_data.get('page_num', 0)
        
        # Decode PDF
        pdf_bytes = base64.b64decode(pdf_base64)
        
        # Add signature
        signed_pdf_bytes = pdf_form_service.add_signature_to_pdf(
            pdf_bytes, signature_base64, signature_type, page_num
        )
        ...
        return Response(
            content=signed_pdf_bytes,
            media_type="application/pdf",
            headers={"Content-Disposition": "inline; filename=i9-signed.pdf"}
        )
...
async def add_signature_to_w4(
    ...
    """Add digital signature to W-4 PDF"""
    try:
        pdf_base64 = signature_data.get('pdf_data')
        signature_base64 = signature_data.get('signature')
        signature_type = signature_data.get('signature_type', 'employee_w4')
        page_num = signature_data.get('page_num', 0)
        ...
        return Response(
            content=signed_pdf_bytes,
            media_type="application/pdf",
            headers={"Content-Disposition": "inline; filename=w4-signed.pdf"}
        )
