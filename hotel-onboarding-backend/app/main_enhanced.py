...
async def approve_application_enhanced(
    ...
    property_obj = database["properties"][application.property_id]
    manager_name = f"{current_user.first_name} {current_user.last_name}".strip()
    onboarding_link = f"{os.getenv('FRONTEND_URL', 'http://localhost:5173')}/onboarding/{onboarding_token_data['token']}"
    ...
    return {
        ...
        "onboarding": {
            ...
            "onboarding_url": f"{os.getenv('FRONTEND_URL', 'http://localhost:5173')}/onboarding/{onboarding_token_data['token']}"
        },
        "talent_pool": {
...
async def approve_application(
    ...
    # Create secure onboarding session with JWT token
