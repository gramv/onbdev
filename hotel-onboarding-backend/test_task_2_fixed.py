#!/usr/bin/env python3
"""
Fixed Test for Task 2: Onboarding Orchestrator Service
Tests both subtasks 2.1 and 2.2 implementation with proper application approval
"""
import sys
import os
import requests
...
def test_onboarding_orchestrator_service():
    ...
    try:
        ...
        # Step 3: First check the application status
        ...
        if app_response.status_code == 200:
            ...
            print(f"✅ Found {len(apps_data['applications'])} applications")
            ...
            if test_app:
                print(f"   Test application status: {test_app['status']}")
                
                # If not approved, approve it first
                if test_app['status'] != 'approved':
                    print("\n3a. Approving test application...")
                    approve_response = requests.post(
                        f"{base_url}/manager/applications/{application_id}/approve",
                        headers={"Authorization": f"Bearer {manager_token}"},
                        json={
                    ...
                    print(f"✅ Application already approved")
            else:
                print(f"❌ Test application {application_id} not found")
                return False
        else:
            print(f"❌ Failed to get applications: {app_response.text}")
            return False
