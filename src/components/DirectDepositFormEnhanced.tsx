...
import { CreditCard, Building, Plus, Trash2, AlertTriangle, Info, Upload, Check, Clock, FileText, Save, Loader2, Bot, Sparkles } from 'lucide-react'
import DocumentUpload from '@/components/DocumentUpload'
import { DocumentType } from '@/types/documents'
import { DocumentMetadata } from '@/services/documentService'
import { useAutoSave } from '@/hooks/useAutoSave'
import { getApiUrl } from '@/config/api'
// Removed routing validation service - now using OCR validation only
...
// Simple ABA checksum validation
function validateABAChecksum(routing: string): boolean {
...
export default function DirectDepositFormEnhanced({
  ...
  // Removed routing validation states - now relying on OCR validation only
  
  // AI OCR validation states
  ...
  // Store base64 image data for OCR validation
  const [voidedCheckBase64, setVoidedCheckBase64] = useState<string>('')
  const [bankLetterBase64, setBankLetterBase64] = useState<string>('')
  ...
  const t = (key: string) => {
    const translations: Record<string, Record<string, string>> = {
      en: {
        ...
        'upload_voided_check_required': 'Please upload a voided check or bank letter to continue',
        'routing_format_valid_upload_check': 'Routing format valid. Upload check to verify with your bank',
        'enter_9_digit_routing': 'Enter 9-digit routing number',
        'ocr_validation_required': 'Bank document validation required',
        'invalid_routing_format': 'Invalid routing number format (must pass ABA checksum)'
      },
      es: {
        ...
        'upload_voided_check_required': 'Por favor cargue un cheque anulado o carta bancaria para continuar',
        'routing_format_valid_upload_check': 'Formato de ruta válido. Cargue cheque para verificar con su banco',
        'enter_9_digit_routing': 'Ingrese número de ruta de 9 dígitos',
        'ocr_validation_required': 'Se requiere validación con documento bancario',
        'invalid_routing_format': 'Formato de número de ruta inválido'
      }
    }
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (formData.paymentMethod === 'direct_deposit') {
      // Require voided check or bank letter upload
      if (!formData.voidedCheckUploaded && !formData.bankLetterUploaded) {
        newErrors['document_required'] = t('upload_voided_check_required')
      }
      ...
      } else if (!validateABAChecksum(formData.primaryAccount.routingNumber)) {
        newErrors['primaryAccount.routingNumber'] = t('invalid_routing_format')
      }
      ...
      if (formData.depositType === 'split') {
        formData.additionalAccounts.forEach((account, index) => {
          ...
          } else if (!validateABAChecksum(account.routingNumber)) {
            newErrors[`additionalAccounts.${index}.routingNumber`] = t('invalid_routing_format')
          }
  ...
  // Removed routing validation - now using OCR validation only
  ...
  const handleInputChange = (field: string, value: any) => {
    ...
    // Removed database routing validation - now using OCR only
  }
  ...
  const validateCheckWithAI = async (document: DocumentMetadata, base64Data: string) => {
    setOcrValidation(prev => ({ ...prev, isProcessing: true }))
    
    try {
      // Use the provided base64 data
      const imageData = base64Data
      
      const response = await fetch(`${getApiUrl()}/onboarding/${employee?.id || 'temp'}/direct-deposit/validate-check`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          manual_data: {
            bank_name: formData.primaryAccount.bankName || '',
            routing_number: formData.primaryAccount.routingNumber || '',
            account_number: formData.primaryAccount.accountNumber || ''
          }
        })
      })
      ...
      if (result.success && result.data) {
        setOcrValidation({
          ...
          processingNotes: result.data.processing_notes
        })
        
        // Auto-apply high confidence suggestions (lowered threshold for routing numbers)
        if (result.data.suggestions?.length > 0) {
          result.data.suggestions.forEach(suggestion => {
            // Lower threshold for routing numbers to 0.75
            const threshold = suggestion.field === 'routing_number' ? 0.75 : 0.9
            if (suggestion.confidence >= threshold) {
              ...
              setFormData(prev => ({
                ...
                primaryAccount: {
                  ...prev.primaryAccount,
                  ...(suggestion.field === 'routing_number' ? { routingNumber: suggestion.suggested_value } :
                      suggestion.field === 'account_number' ? { accountNumber: suggestion.suggested_value } :
                      suggestion.field === 'bank_name' ? { bankName: suggestion.suggested_value } : {})
                }
              }))
            }
          })
      ...
      setOcrValidation({
        isProcessing: false,
        suggestions: [],
        processingNotes: ['Failed to validate check with AI']
      })
    }
  }
              ...
              <div>
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder={t('enter_9_digit_routing')}
                    maxLength={9}
                  />
                  {formData.primaryAccount.routingNumber.length === 9 && 
                   validateABAChecksum(formData.primaryAccount.routingNumber) && (
                    <Check className="absolute right-3 top-3 h-4 w-4 text-blue-600" />
                  )}
                </div>
                ...
                {formData.primaryAccount.routingNumber.length === 9 && 
                 validateABAChecksum(formData.primaryAccount.routingNumber) && (
                  <p className="text-xs mt-1 text-blue-600 flex items-center">
                    ...
                    {t('routing_format_valid_upload_check')}
                  </p>
                )}
              </div>
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <DocumentUpload
                  ...
                  onUploadComplete={async (document, file) => {
                    setFormData(prev => ({ 
                      ...prev, 
                      voidedCheckUploaded: true,
                      voidedCheckDocument: document
                    }))
                    // Convert file to base64 for OCR validation
                    if (file) {
                      try {
                        const base64 = await fileToBase64(file)
                        setVoidedCheckBase64(base64)
                        // Validate check with AI if we have account details entered
                        if (formData.primaryAccount.routingNumber || formData.primaryAccount.accountNumber) {
                          await validateCheckWithAI(document, base64)
                        }
                      } catch (error) {
                        console.error('Error converting file to base64:', error)
                        setOcrValidationError('Failed to process the uploaded file')
                      }
                    }
                  }}
                  onUploadError={(error) => {
                ...
                <DocumentUpload
                  ...
                  onUploadComplete={async (document, file) => {
                    ...
                    // Convert file to base64 for potential future OCR validation
                    if (file) {
                      try {
                        const base64 = await fileToBase64(file)
                        setBankLetterBase64(base64)
                      } catch (error) {
                        console.error('Error converting bank letter to base64:', error)
                      }
                    }
                  }}
                  onUploadError={(error) => {
              ...
              {ocrValidation.suggestions.length > 0 && (
                <Alert className="mt-4 border-blue-200 bg-blue-50">
                  <Bot className="h-4 w-4 text-blue-600" />
                  <AlertDescription>
                    <div className="space-y-2">
                      <div className="font-medium text-blue-900">Validation Results</div>
                      {ocrValidation.suggestions.map((suggestion, idx) => (
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-xs">
                                <div>
                                  ...
                                  <span className="ml-1 font-mono">{suggestion.user_value}</span>
                                </div>
                                <div>
                                  <span className="text-gray-500">System detected:</span>
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                if (suggestion.field === 'routing_number') {
                                  setFormData(prev => ({
                                    ...prev,
                                    primaryAccount: {
                                      ...prev.primaryAccount,
                                      routingNumber: suggestion.suggested_value
                                    }
                                  }))
                                } else if (suggestion.field === 'account_number') {
                                  setFormData(prev => ({
                                    ...prev,
                                    primaryAccount: {
                                      ...prev.primaryAccount,
                                      accountNumber: suggestion.suggested_value
                                    }
                                  }))
                                } else if (suggestion.field === 'bank_name') {
                                  setFormData(prev => ({
                                    ...prev,
                                    primaryAccount: {
                                      ...prev.primaryAccount,
                                      bankName: suggestion.suggested_value
                                    }
                                  }))
                                }
              ...
              {ocrValidation.isProcessing && (
                <Alert className="mt-4">
                  <Loader2 className="h-4 w-4 animate-spin" />
                  <AlertDescription>
                    Processing your document... This may take a few seconds.
                  </AlertDescription>
                </Alert>
              )}
